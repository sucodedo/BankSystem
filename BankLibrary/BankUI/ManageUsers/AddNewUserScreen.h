...
class AddNewUserScree : protected Screen
{
private:
    ...
    static void _ReadUserInfo(User& UserObj)
    {
        ...
        UserObj.Permissions(AddNewUserScree::_ReadPermissions());
    }
    ...
    static int _ReadPermissions()
    {
        ...
        if (AskForPer == 'y' || AskForPer == 'Y')
            Permissions += User::enPermissions::pRegisterLogin;
...
public:
    ...
    {
        AddNewUserScree::_ClearScreen();
        AddNewUserScree::_ScreenHeader("\t\tAdd New User Screen");
        ...
        User AddUser = User::GetAddNewUser(UserName);
        
        _ReadUserInfo(AddUser);

        _PrintUserInfo(AddUser);

        User::enSaveResults SaveResult;
        SaveResult = AddUser.Save();

        switch(SaveResult)
        {
            case User::enSaveResults::svSucceeded:
                cout << "\nUser added succeessfully.\n" << endl;
                _PrintUserInfo(AddUser);
                break;
            case User::enSaveResults::svFailedEmptyObj:
                cout << "User added Failed" << endl;
                break;
            case User::enSaveResults::svFailedUserNameExists:
